                ...
                if (!$_GET) {
                        ...
                        foreach ($data as $key => $dat) {
                                if (preg_match("%^(?:(?:https?|ftp)://)(?:\S+(?::\S*)?@|\d{1,3}(?:\.\d{1,3}){3}|(?:(?:[a-z\d\x{00a1}-\x{ffff}]+-?)*[a-z\d\x{00a1}-\x{ffff}]+)(?:\.(?:[a-z\d\x{00a1}-\x{ffff}]+-?)*[a-z\d\x{00a1}-\x{ffff}]+)*(?:\.[a-z\x{00a1}-\x{ffff}]{2,6}))(?::\d+)?(?:[^\s]*)?$%iu", $dat, $match)) {
                                        ...
                                        $content .= "<li class=\"list-group-item d-flex justify-content-between align-items-center\">" . nl2br($dat) . "<a href=\"not.php?id={$key}\" class=\"btn btn-warning\">Düzenle</a></li>";
                                }
                        }
                } else {
                        $data = json_decode(file_get_contents("not.json"), true);
...
?>
                ...
                <head>
                        ...
                        <meta name="description" content="Not uygulaması">
                                ...
                                <div class="row"><i class="bi-alarm"></i>
                                        <div class="form-group">
                                                <form action="not.php" method="POST">
                                                        <textarea class="form-control" id="not" placeholder="not yaz" name="not" rows="5"><?php if (is_string($data) && strlen($data) > 0) {
                                                                                                                                                        echo $data;
                                                                                                                                                } ?></textarea>
